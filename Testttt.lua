-- ============================================================================
-- NOVA UI v3.3 — SAFE MODE EDITION
-- FIX UTAMA: Hook __namecall TIDAK di-install otomatis.
--            Hook hanya dipasang saat user toggle "Magic Bullet" ON.
--            Semua loop dibungkus pcall → 1 fitur error tidak matikan UI.
-- ============================================================================
...
function Library.CreateTab(tabName, icon)
    ...
    function TabObject.AddDropdown(text, options, default, callback)
        ...
        local function SetOpen(s)
            ...
            if not open then task.delay(0.25, function() if not open then ListScroll.Visible = false end end) end
        end
        ...
        for _, opt in ipairs(options) do
            local OptBtn = Utils.Create("TextButton", { Parent=ListScroll, Size=UDim2.new(1,0,0,26),
                ...
                Utils.Create("TextLabel", { Position=UDim2.new(0,10,0,0), Size=UDim2.new(1,-30,1,0),
            ...
            OptBtn.MouseButton1Click:Connect(function()
                ...
                for _, data in ipairs(optionBtns) do data.Check.Visible = false; data.Label.TextColor3 = TH.TextSecondary end
                OptBtn.Check.Visible = true; label.TextColor3 = TH.TextPrimary
                SetOpen(false)
                pcall(callback, selected)
            end)
            optionBtns[#optionBtns+1] = { Btn=OptBtn, Label=label, Check=OptBtn.Check, Value=opt }
        end
        for _, d in ipairs(optionBtns) do
            if d.Value == selected then d.Check.Visible = true; d.Label.TextColor3 = TH.TextPrimary end
        end
        local function Set(value, silent)
            ...
            for _, d in ipairs(optionBtns) do
                ...
                d.Check.Visible = m
                d.Label.TextColor3 = m and TH.TextPrimary or TH.TextSecondary
            end
        ...
        table.insert(TabObject.Elements, { Name=text, Instance=Frame })
        return { Frame=Frame, Set=Set, Get=function() return selected end }
    end
...
AddConn(RunService.RenderStepped:Connect(function()
    Safe("Spectate", function()
        if State.Spectate and State.SpectateTarget and State.SpectateTarget.Character then
...
if #playerList == 0 then playerList = {"None"} end
TpTab.AddDropdown("Select Player", playerList, nil, function(sel)
    for _, p in ipairs(Players:GetPlayers()) do
        if p.Name == sel then targetPlayer = p; break end
    end
end)
TpTab.AddButton("Teleport to Selected Player", function()
    ...
    else Notify("Teleport", "Pilih player dulu", 4, "Warning") end
end)
...
TpTab.AddButton("Goto Random Player", function()
    ...
    if t and t ~= LocalPlayer and t.Character and LocalPlayer.Character then
